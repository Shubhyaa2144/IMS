...
class supplierClass:
    def __init__(self, root):
        ...
        lbl_search=Label(self.root,text="Invoice No.",bg="white",font=("goudy oid style",15))
        ...
        btn_search=Button(self.root,text="Search",command=self.serach,font=("goudy old style",15),bg="#4caf50",fg="white",cursor=("hand2")).place(x=980,y=79,width=100,height=28)
        ...
        self.txt_desc.place(x=180,y=220,width=470,height=120)
    ...
    def add(self):
        ...
        try:
            if self.var_sup_invoice.get() == "":
                ...
                row = cur.fetchone()
                if row is not None:
                    messagebox.showerror("Error", "Invoice No. already exists, try a different one", parent=self.root)
                else:
    ...
    def serach(self):
        ...
        try:
            if self.var_searchtxt.get()=="":
                ...
                cur.execute("SELECT * FROM supplier WHERE invoice=? ",(self.var_searchby.get(),)) 
                row=cur.fetchone()
                if row!=None:
                    self.supplierTable.delete(*self.supplierTable.get_children())
