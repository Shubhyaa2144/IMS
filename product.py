...
class productClass:
    def __init__(self, root):
        ...
        product_Frame.place(x=10,y=10,width=450,height=480)
         ...
         #====title====
        title=Label(product_Frame,text="Manage Product Detalis",font=("goudy old style",18),bg="#0f4d7d",fg="white").pack(side=TOP,fill=X)
        ...
        lbl_qyt=Label(product_Frame,text="Quantity",font=("goudy old style",18),bg="white").place(x=30,y=260)
        title_status=Label(product_Frame,text="Status",font=("goudy old style",18),bg="white").place(x=30,y=310)
        
        
        #====column2=====

        cmd_cat=ttk.Combobox(product_Frame,textvariable=self.var_cat,values=self.cat_list,state='readonly',justify=CENTER,font=("goudy oid style",15))
        cmd_cat.place(x=150,y=60,width=200)
        cmd_cat.current(0)

        cmd_sup=ttk.Combobox(product_Frame,textvariable=self.var_sup,values=self.sup_list,state='readonly',justify=CENTER,font=("goudy oid style",15))
        cmd_sup.place(x=150,y=110,width=200)
        cmd_sup.current(0)

        txt_name=Entry(product_Frame,textvariable=self.var_name,font=("goudy oid style",15),bg="lightyellow").place(x=150,y=160,width=200)
        txt_price=Entry(product_Frame,textvariable=self.var_price,font=("goudy oid style",15),bg="lightyellow").place(x=150,y=210,width=200)
        txt_qty=Entry(product_Frame,textvariable=self.var_qty,font=("goudy oid style",15),bg="lightyellow").place(x=150,y=260,width=200)
        
        cmd_status=ttk.Combobox(product_Frame,textvariable=self.var_status,values=("Active", "Inactive"),state='readonly',justify=CENTER,font=("goudy oid style",15))
        ...
        cmd_search=ttk.Combobox(SearchFrame,textvariable=self.var_searchby,values=("Select","Category","Supplier","Name"),state='readonly',justify=CENTER,font=("goudy oid style",15))
        cmd_search.place(x=10,y=10,width=180)
        cmd_search.current(0)

        txt_search=Entry(SearchFrame,textvariable=self.var_searchtxt,font=("goudy old style",15),bg="lightyellow").place(x=200,y=10)
        btn_search=Button(SearchFrame,text="Search",command=self.serach,font=("goudy old style",15),bg="#4caf50",fg="white",cursor=("hand2")).place(x=410,y=9,width=150,height=30)
        ...
        try:
            if self.var_cat.get() == "Select" or self.var_cat.get() == "Empty" or self.var_sup.get() == "Select" or self.var_name.get() == "":
                ...
                if row is not None:
                    ...
                    self.var_sup.get(),
                    self.var_name.get(),
                    self.var_price.get(),   # Ensure this exists
                    self.var_qty.get(),     # Ensure this exists
                    self.var_status.get()   # Ensure this exists
                ))
                con.commit()
