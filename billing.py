...
class BillClass:
    def __init__(self,root):
        ...
        lbl_search=Entry(ProductFrame2,textvariable=self.var_search,font=("times new roman",15,),bg="lightyellow").place(x=128,y=47,width=150,height=22)
        btn_search=Button(ProductFrame2,text="Search",command=self.serach,font=("goudy old style",15),bg="#2196f3",fg="white",cursor="hand2").place(x=285,y=45,width=100,height=25)
        btn_show_all=Button(ProductFrame2,text="Show All",command=self.show,font=("goudy old style",15),bg="#083135",fg="white",cursor="hand2").place(x=285,y=10,width=100,height=25)
        ...
        cTitle = Label(CustomerFrame, text="Customer Details", font=("goudy old style", 20, "bold"), bg="lightgray")
        cTitle.pack(side=TOP, fill=X)

        lbl_name=Label(CustomerFrame,text="Name",font=("times new roman",15,"bold"),bg="white").place(x=5,y=39)
        lbl_name=Entry(CustomerFrame,textvariable=self.var_cname,font=("times new roman",13,),bg="lightyellow").place(x=80,y=39,width=180)
        
        lbl_Contact=Label(CustomerFrame,text="Contact No.",font=("times new roman",15,"bold"),bg="white").place(x=270,y=39)
        lbl_Contact=Entry(CustomerFrame,textvariable=self.var_contact,font=("times new roman",13,),bg="lightyellow").place(x=380,y=39,width=140)
        #======== cal_Cart Frame=======
        Cal_Cart_Frame=Frame(self.root,bd=9,relief=RIDGE,bg="white")
        ...
        btn_3=Button(Cal_Frame,text='1',font=('arial',15,'bold'),command=lambda:self.get_input(1),bd=5,width=4,pady=11,cursor="hand2").grid(row=3,column=0)
        ...
        btn_1=Button(Cal_Frame,text='3',font=('arial',15,'bold'),command=lambda:self.get_input(3),bd=5,width=4,pady=11,cursor="hand2").grid(row=3,column=2)
        ...
        btn_eq=Button(Cal_Frame,text='=',font=('arial',15,'bold'),command=self.perfrom_cal,bd=5,width=4,pady=15,cursor="hand2").grid(row=4,column=2)
        ...
        btn_generate=Button(billMenuFrame,text='Generate/SaveBill',command=self.generate_bill,cursor="hand2",font=("goudy old style",15,"bold"),bg="#009688",fg="white")
        btn_generate.place(x=246,y=80,width=160,height=50)


        footer=Label(self.root,text="IMS-Inventory Management System | Developed By Shubham\nFor any Technical Issue contact: 9699384019",font=("times new roman",11),bg="#4d636d",fg="white").pack(side=BOTTOM,fill=X)
        self.show()
        #self.bill_top()
...
#========================All functions==================================
    ...
    def serach(self):
        ...
        try:
            if self.var_search.get()=="":
                messagebox.showerror("Error","Serach input should be required",parent=self.root)
            else:
    ...
    def bill_top(self):
        ...
        bill_top_temp=f'''
\t\tXYZ-Inventory
\t Phone No. 96993***** , Delhi-125001
{str("="*47)}
 Customer Name: {self.var_cname.get()}
ph no. {str(self.invoice)}\t\t\tDate: {str(time.strftime("%d/%m/%Y"))}
{str("="*47)}
    ...
    def bill_middle(self):
        ...
        try:
            for row in self.cart_list:
                pid = row[0]
                name = row[1]
                qty = int(row[4]) - int(row[3])  # Remaining stock
            ...
            # Update qty and status in product table
    ...
    def print_bill(self):
        if self.chk_print == 1:
            ...
            with os.fdopen(fd, 'w', encoding='utf-8') as tmp_file:
                tmp_file.write(self.txt_bill_area.get('1.0', END))
        
        try:
            os.startfile(path, 'print')  # Works only on Windows
